...
def test_load_image(test_file, CLAHE_load, CLAHE_clip_load, CLAHE_grid_size_load,invert_flag, target_size=(256,256)):
    ...
    print(average)
    # lower average is darger, 0 is black. 255 is white.
    if(average > 50):
        print('average =', average)
        print('needs reversal')
        # this image needs to be reversed
        img = cv2.bitwise_not(img)
        
        invert_flag.append(1)
    else:
        invert_flag.append(0)
    ...
    # this makes mask generation much worse
    img = cv2.equalizeHist(img)
    
    # image 071 is still a problem
    # try CLAHE
...
def test_generator(test_files, CLAHE_load, CLAHE_clip_load, CLAHE_grid_size_load,invert_flag, target_size=(256,256)):
    for test_file in test_files:
        yield test_load_image(test_file, CLAHE_load, CLAHE_clip_load, CLAHE_grid_size_load,invert_flag, target_size)
...
#     #closed = flood_fill(cv2.morphologyEx(img, cv2.MORPH_CLOSE, kernel))
    
#     closed = cv2.morphologyEx(img, cv2.MORPH_CLOSE, kernel)
         
#     contours, hierarchy, _ = cv2.findContours(closed, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
...
#     #closed = cv2.morphologyEx(closed, cv2.MORPH_CLOSE,  cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(10,80)))        
...
def segment(sourcepath, maskpath, destpath, CLAHE_save, CLAHE_clip_save, CLAHE_grid_size_save,invert_flag):   
    #
...
def segment_filtered_images(model_path, images_path, output_masks_path, output_segmentation_path, 
    ...
    for i in range(n_batches):
        ...
        test_gen = test_generator(files[batch_idx_in:batch_idx_fi],  
                                  CLAHE_load, CLAHE_clip_load, CLAHE_grid_size_load,invert_flag,
                                  target_size=(512,512))
    ...
    segment((images_path + "/*.*"),output_masks_path,output_segmentation_path, 
             CLAHE_save, CLAHE_clip_save, CLAHE_grid_size_save, invert_flag)
